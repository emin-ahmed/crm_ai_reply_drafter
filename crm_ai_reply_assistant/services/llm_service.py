...
class AiReplyService(models.AbstractModel):
    ...
    def _get_config(self):
        ...
        provider = get_param('crm_ai_reply_assistant.provider', 'anthropic')
        keys = {
            'anthropic': 'crm_ai_reply_assistant.anthropic_api_key',
            'openai': 'crm_ai_reply_assistant.openai_api_key',
        }
        ...
        if provider == 'ollama':
            base_url = get_param('crm_ai_reply_assistant.ollama_base_url') or \
                'http://localhost:11434'
        elif provider == 'openai':
            base_url = get_param('crm_ai_reply_assistant.openai_base_url') or False
        else:
            base_url = False
        return {
            'provider': provider,
            'model': get_param('crm_ai_reply_assistant.model') or
            providers.DEFAULT_MODELS.get(provider),
            'api_key': get_param(keys.get(provider, '')) or False,
            'base_url': base_url,
            'system_prompt': get_param('crm_ai_reply_assistant.system_prompt') or '',
            'max_messages': int(
                get_param('crm_ai_reply_assistant.max_context_messages') or 20),
        }
